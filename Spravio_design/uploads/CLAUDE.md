...
| Prioridade | Issue |
|------------|-------|
| 🔴 CRÍTICO | Senhas em plaintext (`auth/route.ts:40`) |
| 🟠 Médio | Rotas sem auth guards (GET /projects, etc.) |
| 🟠 Médio | Sem migrations formais fases 10-16 |
...
2. **NUNCA** use `any` — use `unknown` + type guards
3. **SEMPRE** cache chamadas externas (Redis TTL 300s)
4. **NUNCA** `console.log` em produção — use Pino
5. **SEMPRE** Zod em inputs E outputs de rotas
6. **NUNCA** SQL raw — use Prisma client
7. **SEMPRE** loading.tsx + error.tsx em rotas
8. **NUNCA** npm/yarn — use pnpm

---

*Última atualização: 2026-04-04*
*Branch: main | Commit: 880aded*
