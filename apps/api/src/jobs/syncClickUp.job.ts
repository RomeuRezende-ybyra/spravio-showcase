...
import {
  ...
  mapClickUpList,
  mapClickUpTask,
} from '../integrations/clickup/index.js'
...
export interface SyncClickUpPayload {
  projectId: string
  clickupSpaceId: string
}

export async function syncClickUpProcessor(job: Job<SyncClickUpPayload>) {
  const { projectId, clickupSpaceId } = job.data
  ...
  try {
    ...
    const members = await getSpaceMembers(clickupSpaceId)
    const developerMap = new Map<string, string>() // clickupUserId -> developerId

    for (const member of members) {
      const args = mapClickUpMember(member)
      const dev = await prisma.developer.upsert(args)
      developerMap.set(String(member.user.id), dev.id)
    ...
    const lists = await getAllSpaceLists(clickupSpaceId)
    const listMap = new Map<string, string>() // clickupListId -> sprintId

    for (const list of lists) {
      const args = mapClickUpList(list, projectId)
      const sprint = await prisma.sprint.upsert(args)
      listMap.set(list.id, sprint.id)
      itemsSynced++
    }
    await job.updateProgress(50)

    // 3. Sync tasks as issues
    const tasks = await getSpaceTasks(clickupSpaceId)

    // Track points per sprint for updating totals
    const sprintPoints = new Map<string, { total: number; completed: number }>()

    for (const task of tasks) {
      // Skip subtasks (parent is set)
      if (task.parent) continue

      const { data, assigneeUserId, listId } = mapClickUpTask(task)

      const sprintId = listMap.get(listId)
      if (!sprintId) continue // Task not in a known list
